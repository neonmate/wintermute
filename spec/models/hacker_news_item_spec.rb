describe HackerNewsItem do
  ...
  describe '#validate' do
    ...
    it { is_expected.to_not allow_values(nil, '', 'other').for(:type) }
    ...
    it { is_expected.to_not allow_values(nil, '').for(:deleted) }

    it { is_expected.to allow_values(true, false).for(:dead) }
    it { is_expected.to_not allow_values(nil, '').for(:dead) }
